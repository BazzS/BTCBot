...
def getMessage(message):
    bot.send_message(message.from_user.id,"Процент прибыли",reply_markup=keyboard)
...
def callback_func(query):
    ...
    bot.send_message(query.message.chat.id, 'Введите число:')
...
def number(message, value):
    if message.text.replace('.', '').isdigit():
        ...
        bot.send_message(message.chat.id, 'Введите именно число:')
